...
These fluctuations are driven by the random thermal motions of the electrons and the collective radiation they create is known as incoherent scatter \cite{kudeki:milla:1}. Another term for this is \textit{non-collective,} scattering which specifically entails the following relation,
...
\noindent where $\lambda_{e}$ is the electron Debye length, and $k=|\mathbf{k}|$. This will cause the spectrum to show the collective effects of the velocity distribution of the particles \cite{sheffield2010}.

Although the scattering is driven through a random process it can give a number of different pieces of information about a plasma, especially in the ionosphere. Due to the electrical interaction between the ions and electrons a correlation structure develops and creating a shaped Doppler spectrum that a radar can estimate. Stated another way this is an estimate of the power spectral density of the electron density fluctuations across time, $\langle \left|n_e(\mathbf{k},\omega)\right|^2\rangle$.
...
\end{equation}
 ...
 \noindent where $k_0$ is the wavenumber, $\omega$ is the radian frequency of the wave, $K_b$ is Boltzman's constant, $m_i$ is the mass of the ion species, $T_e$ and $T_i$ are the temperatures of the electron and ions \cite{chen1984introduction}. Other plasma wave modes can be viewed but ion acoustic mode returns the most power and can give information on both electron and ions.

As mentioned before this process is inherently stochastic in nature. In order to measure this spectrum with any confidence a number of samples of this process must be averaged together \cite{Diaz:2008co}. Within this thesis and often in other radar publications the samples of this random process are often referred to as "pulses" to distinguish this from samples in signal processing applications \cite{dtsp:openhiem}. A demonstration of the convergence of a measurement going to the spectrum, using a periodogram estimator \cite{kay1988modern}, can be seen in Figure \ref{fig:ispecch2ave}.
...
The second method, entails forming a measurement of the autocorrelation function (ACF) with in the IPP. A DFT across the lags is taken to then get the Doppler spectrum The formation of this spectrum is the same as forming Wigner-Ville distribution along range \cite{TFAcohen}. Because phase coherency is not needed across pulses this is considered an \text{incoherent processing} technique \cite{richards2014fundamentals,richards2010principles,richards2014principles,skolnik2008radar}. It is also noted that ISR received its name due to the physical definition of incoherent scatter as explained in Section \ref{sec:incohscat} \cite{gordon58,dougherty:farley1960}.
