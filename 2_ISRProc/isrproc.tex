...
\noindent where $\mathbf{k}_s$ and $\mathbf{k}_i$ are the wavenumbers for scattered and incident waves respectively \cite{sheffield2010}. The Bragg vector is the frequency parameter in $n_e(\mathbf{k},t)$, where is the spatial Fourier transform of the $n_e(\mathbf{r},t)$ where $\mathbf{r}$ is the position vector.
...
The IS spectrum has a number of different sections separated in frequency but for this thesis we will focus on the "ion line." This naming is due to the IS spectrum is showing a damped version of ion acoustic waves,
...
Using a pulsed radar systems one can measure Doppler in a two main ways. A more common
