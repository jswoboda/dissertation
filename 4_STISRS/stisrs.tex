...
Once the theoretical spectrum for a given scattering volume has been calculated, an appropriate spectral shaping filter is created. The method to create the filter given a desired spectrum or ACF can be done in a number of ways \cite{Kasdin:1995wi}. The implementation in SimISR creates an infinite impulse response filter. 
...
%% JPS: added statement about errors\begin{figure}[!t]
\begin{figure}[!t]
\centering
...
Using a beam pattern similar to the one seen in the right panel of Figure \ref{fig:background1}, we use SimISR to explore how an electronically scanned ISR may reconstruct this dynamic auroral arc system.  For the purposes of illustration, we use the contrived case where the radar spatial beam pattern is defined to be in the plane of convection.  It is also assumed that the ion species present (particularly their individual masses and relative concentrations) are known. This is a common use case in ISR fitting, as allowing ion concentrations to be free parameters can in some situations allow for non-unique solutions depending on the ion species that are present. If the fixed, apriori assumed composition ratios between the different ion species are incorrect, this can lead to errors in the final parameter estimates, with ion temperature particularly affected. For the lower F region ionosphere, parameter distortions can occur between 150-250 km where the ionosphere changes from NO$^+$ dominated to O$^+$ dominated \cite{Zettergren:2011ej, Blelly:2010gf}. We also note that as the field aligned current passes through the simulated field, an influx of NO$^+$ appears in the region of rapid ion mass transition for this simulation, potentially violating ion composition assumptions \cite{Perry:2015jf}.
...
%% PJE: fill in the XXX
%% JPS: Filled in
After the data was fitted it was interpolated using natural neighbors
...
and plotted using the GeoData API \cite{john_swoboda_2016_154536}.
...
The input plasma parameters and the results of SimISR simulation can be seen in Figure \ref{fig:3dparams}. The enhancements in ion temperature can clearly be seen in the reconstruction, although they do look wider than the input which is likely due to the spatial ambiguity funciton. 
%% PJE: vague - reword or omit.  If you say e.g. "appear wider" someone is going to ask "why?"
%% JPS: Added statement about ambiguity.
The electron temperature enhancement is also visible but less pronounced compared to the input spatial variations. There does seem to be some large electron temperature enhancements near 200 km altitude, although this could be due to improper fits brought on by changes in ion composition at low altitudes. Lastly the depletions in electron density are visible as well in the same pattern as seen on the input parameters. In the end this qualitatively that SimISR can reconstruct a 3-D field of plasma state parameters.
%% PJE: all of this leave the impression of "we did this, we saw that" qualitative statements without any real insight that results.  I would think carefully about the real message you want to get across in this use case (and simplify + reword accordingly), because I have a hard time as a reader figuring it out.
%% JPS: I wanted to add this example to show the reader of the ability to reconstruction a 3-D field of plasma parameters.
\begin{figure}[!t]
\centering
