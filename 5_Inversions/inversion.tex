...
\noindent where $\mathbf{0}$ is a matrix of zeros the same size as $\mathbf{K}$. This representation has some advantages as instead of inverting the whole matrix, different blocks can be inverted at any one time. However, this still leads to rather large matrices to invert because the number of rows grow with the number of number of times integrated. 
...
\noindent This constraint is generally referred to as \textit{total variations} and has been used in image processing \citep{Rudin:1992kn}

\section{Results}
\label{sec:results}
This section we will present two different examples to show our inversion algorithm. The first case will show how the the inversion methods work on reversing the linear process of the space time ambiguity function. The second example will be used on data from SimISR (Simulator for ISR), which was detailed in Chapter~\ref{chapter:stisrs}.
